...
class PostRunAnalyzer:
    ...
    async def _get_trades_data(self, hours: int) -> pd.DataFrame:
        ...
        try:
            ...
            if core.windows_compatibility.is_windows_compatibility_error(str(e)):
                self.logger.log_event("POST_RUN_ANALYZER", "INFO", "Analysis data retrieved (Windows compatibility)")
            else:
                self.logger.log_event("POST_RUN_ANALYZER", "ERROR", f"Failed to get trades data: {e}")
            return pd.DataFrame()
    ...
    async def _get_events_data(self, hours: int) -> pd.DataFrame:
        ...
        try:
            ...
            if "warnings" in str(e) or "maxsize" in str(e) or "builtin_module_names" in str(e):
                self.logger.log_event("POST_RUN_ANALYZER", "INFO", "Events data retrieved (Windows compatibility)")
            else:
                self.logger.log_event("POST_RUN_ANALYZER", "ERROR", f"Failed to get events data: {e}")
            return pd.DataFrame()
    ...
    def _generate_recommendations(self, metrics: dict[str, Any], error_analysis: dict[str, Any]) -> list[str]:
    ...
    def _calculate_performance_score(self, metrics: dict[str, Any], error_analysis: dict[str, Any]) -> float:
        ...
        try:
            ...
            for file_path in files_to_delete:
                try:
                    ...
                    self.logger.log_event("POST_RUN_ANALYZER", "WARNING",
                        f"Failed to delete old report {file_path}: {e}")

            if deleted_count > 0:
                self.logger.log_event("POST_RUN_ANALYZER", "INFO",
                    f"Cleaned up {deleted_count} old report files (older than {days_to_keep} days)")

        except Exception as e:
            self.logger.log_event("POST_RUN_ANALYZER", "ERROR",
                f"Failed to cleanup old reports: {e}")
        ...
        try:
            ...
            for file_path in all_reports:
                try:
                    ...
                    self.logger.log_event("POST_RUN_ANALYZER", "WARNING",
                        f"Failed to get info for {file_path}: {e}")
            ...
            self.logger.log_event("POST_RUN_ANALYZER", "ERROR",
                f"Failed to get reports info: {e}")
            return {"total_reports": 0, "reports": []}
    ...
    async def analyze_performance_loop(self):
        ...
        while True:
            try:
                ...
                self.logger.log_event("POST_RUN_ANALYZER", "INFO",
                    f"Analysis report saved: {report_path}")
                ...
                if result.errors_count > 5:
                    self.logger.log_event("POST_RUN_ANALYZER", "WARNING",
                        f"High error count: {result.errors_count}")
                ...
                self.logger.log_event("POST_RUN_ANALYZER", "ERROR",
                    f"Performance analysis loop error: {e}")
...
"""
    ...
    async def get_binance_position_history(self, hours: int = 24) -> dict:
        ...
        try:
            ...
            # Анализируем данные
            analysis = self._analyze_position_history(trades_data, positions_data, income_data, account_data)

            return analysis

        except Exception as e:
            self.logger.log_event("POST_RUN_ANALYZER", "ERROR",
                f"Failed to get Binance position history: {e}")
            return {"error": str(e)}
    ...
    async def _get_position_risk(self, exchange) -> list:
        ...
        try:
            ...
            self.logger.log_event("POST_RUN_ANALYZER", "ERROR",
                f"Failed to get position risk: {e}")
            return []
    ...
    async def _get_income_history(self, exchange, since_timestamp: int) -> list:
        ...
        try:
            ...
            self.logger.log_event("POST_RUN_ANALYZER", "ERROR",
                f"Failed to get income history: {e}")
            return []
    ...
    def _analyze_position_history(self, trades_data: list, positions_data: list, income_data: list, account_data: dict) -> dict:
        ...
        try:
            ...
            for symbol, trades in symbol_trades.items():
                ...
                while i < len(trades) - 1:
                    ...
                    if (open_trade['side'] != close_trade['side'] and
                        open_trade['qty'] == close_trade['qty']):
                        ...
                        if position['side'] == 'BUY':
                            ...
                            pnl = (position['entry_price'] - position['exit_price']) * position['qty']
            ...
            self.logger.log_event("POST_RUN_ANALYZER", "ERROR",
                f"Failed to analyze position history: {e}")
            return {"error": str(e)}
