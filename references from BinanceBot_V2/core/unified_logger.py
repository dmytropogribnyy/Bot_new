...
class UnifiedLogger:
            ...
            if self.db_path.exists():
                ...
                with self.lock, sqlite3.connect(self.db_path) as conn:
                    ...
                    for table in ['trades', 'entry_attempts', 'tp_sl_events', 'events']:
                        c.execute(f"DELETE FROM {table} WHERE timestamp < ?", (cutoff_date.isoformat(),))
                    conn.commit()
    ...
    def _rotate_logs(self):
        ...
        try:
            backup_path = self.db_path.with_suffix(f'.backup_{datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")}.db')
            ...
            with self.lock, sqlite3.connect(self.db_path) as conn:
                ...
                for table in ['trades', 'entry_attempts', 'tp_sl_events', 'events']:
                    ...
                    c.execute(f"DELETE FROM {table} WHERE id NOT IN (SELECT id FROM {table} ORDER BY id DESC LIMIT 1000)")
                conn.commit()
    ...
    def log_event(self, component: str, level: str, message: str, details: Any = None, channels: list = None):
        ...
        if component in self.log_rate_limit:
            ...
            if time_since_last < self.min_log_interval and level.upper() not in ['ERROR', 'CRITICAL']:
        ...
        console_msg = f"{color}[{formatted_time}] {emoji} {level.upper()}{component_str} {message}{reset}"
        print(console_msg)
    ...
    def log_analysis_event(self, analysis_type: str, message: str, details: dict = None):
        ...
        self.log_event("ANALYSIS", "INFO", f"{analysis_type}: {message}", details,
    ...
    def log_symbol_analysis(self, symbol: str, analysis_type: str, details: dict = None):
        ...
        # Логируем в runtime.log и БД
        self.log_event("SYMBOL_ANALYSIS", "INFO", message, details, [LogLevel.FILE, LogLevel.DATABASE])
    ...
    def send_alert(self, message: str, level: str = "INFO", details: Any = None):
        ...
        try:
            # Логируем алерт
            self.log_event("ALERT", level, message, details, channels=["telegram", "terminal", "file"])
    ...
    def send_message(self, message: str, level: str = "INFO", details: Any = None):
        ...
        try:
            # Логируем сообщение
            self.log_event("MESSAGE", level, message, details, channels=["telegram", "terminal", "file"])
