...
class EdgeCaseHandler:
        ...
        if not self._edge_case_state["high_volatility_mode"]:
            ...
            for callback in self._edge_case_callbacks["high_volatility"]:
                try:
                    ...
                    self.logger.log_event(
                        "EDGE_CASE", "ERROR", f"❌ High volatility callback error: {e}"
                    )
        ...
        if not self._edge_case_state["extreme_volatility_mode"]:
            ...
            for callback in self._edge_case_callbacks["extreme_volatility"]:
                try:
                    ...
                    self.logger.log_event(
                        "EDGE_CASE", "ERROR", f"❌ Extreme volatility callback error: {e}"
                    )
        ...
        if not self._edge_case_state["market_crash_detected"]:
            ...
            for callback in self._edge_case_callbacks["market_crash"]:
                try:
                    ...
                    self.logger.log_event(
                        "EDGE_CASE", "ERROR", f"❌ Market crash callback error: {e}"
                    )
    ...
    async def _handle_spread_widening(self, symbol: str, spread_pct: float):
        ...
        self.logger.log_event(
            "EDGE_CASE", "WARNING", f"📈 Spread widening detected for {symbol}: {spread_pct:.2%}"
        )
    ...
    async def analyze_network_conditions(
        self, response_time: float, success: bool, error_type: str | None = None
    ) -> dict[str, Any]:
    ...
    async def _handle_network_latency(self, response_time: float):
        ...
        if not self._edge_case_state["network_issues"]:
            ...
            self.logger.log_event(
                "EDGE_CASE", "WARNING", f"🌐 High network latency detected: {response_time:.0f}ms"
            )
    ...
    async def _handle_network_timeout(self, response_time: float):
        ...
        self.logger.log_event(
            "EDGE_CASE", "ERROR", f"⏰ Network timeout detected: {response_time:.0f}ms"
        )
    ...
    async def initiate_recovery(self):
        ...
        try:
            ...
            self._adaptive_measures["position_size_reduction"] = min(
                1.0, self._adaptive_measures["position_size_reduction"] * 1.2
            )
            self._adaptive_measures["risk_multiplier"] = min(
                1.0, self._adaptive_measures["risk_multiplier"] * 1.2
            )
            self._adaptive_measures["trading_frequency_reduction"] = min(
                1.0, self._adaptive_measures["trading_frequency_reduction"] * 1.2
            )
            self._adaptive_measures["leverage_reduction"] = min(
                1.0, self._adaptive_measures["leverage_reduction"] * 1.1
            )
