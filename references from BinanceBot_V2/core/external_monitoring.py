...
class ExternalMonitoring:
    ...
    def __init__(self, config: dict[str, Any], logger: UnifiedLogger):
        ...
        self.prometheus_enabled = (
            config.get("external_monitoring", {}).get("prometheus", {}).get("enabled", False)
        )
        self.grafana_enabled = (
            config.get("external_monitoring", {}).get("grafana", {}).get("enabled", False)
        )
        self.webhook_enabled = (
            config.get("external_monitoring", {}).get("webhook", {}).get("enabled", False)
        )

        # Prometheus settings
        self.prometheus_url = (
            config.get("external_monitoring", {}).get("prometheus", {}).get("url", "")
        )
        self.prometheus_job = (
            config.get("external_monitoring", {})
            .get("prometheus", {})
            .get("job_name", "binance_bot")
        )

        # Grafana settings
        self.grafana_url = config.get("external_monitoring", {}).get("grafana", {}).get("url", "")
        self.grafana_api_key = (
            config.get("external_monitoring", {}).get("grafana", {}).get("api_key", "")
        )

        # Webhook settings
        self.webhook_url = config.get("external_monitoring", {}).get("webhook", {}).get("url", "")
        self.webhook_headers = (
            config.get("external_monitoring", {}).get("webhook", {}).get("headers", {})
        )
    ...
    def set_dependencies(
        self, metrics_aggregator: MetricsAggregator, performance_monitor: PerformanceMonitor
    ):
    ...
    async def start(self):
        ...
        self.logger.log_runtime_status(
            "EXTERNAL_MONITORING_STARTED", f"Export interval: {self.export_interval}s"
        )
    ...
    async def _export_loop(self):
        ...
        while self.running:
            try:
                ...
                self.logger.log_runtime_status(
                    "EXTERNAL_MONITORING_ERROR", f"Export error: {str(e)}"
                )
        ...
        if tasks:
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    self.logger.log_runtime_status(
                        "EXTERNAL_MONITORING_EXPORT_ERROR", f"Export {i + 1} failed: {str(result)}"
                    )
                else:
                    self.logger.log_runtime_status(
    ...
    async def _gather_metrics(self) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.log_runtime_status(
                "EXTERNAL_MONITORING_GATHER_ERROR", f"Error gathering metrics: {str(e)}"
            )
    ...
    def _get_cpu_usage(self) -> dict[str, float]:
        ...
        try:
            ...
            return {"percent": cpu_percent, "count": psutil.cpu_count()}
        except ImportError:
            return {"error": "psutil not available"}
        except Exception:
    ...
    def _format_prometheus_metrics(self, metrics: dict[str, Any]) -> str:
        ...
        if "system_metrics" in metrics:
            ...
            if "memory_usage" in system and "percent" in system["memory_usage"]:
                lines.append(
                    f"binance_bot_memory_usage_percent{{{job_label}}} {system['memory_usage']['percent']}"
                )

            if "cpu_usage" in system and "percent" in system["cpu_usage"]:
                lines.append(
                    f"binance_bot_cpu_usage_percent{{{job_label}}} {system['cpu_usage']['percent']}"
                )
    ...
    async def _export_to_webhook(self, metrics: dict[str, Any]):
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    self.webhook_url, json=webhook_data, headers=self.webhook_headers
                ) as response:
                    if response.status not in [200, 201, 204]:
    ...
    async def manual_export(self) -> bool:
        ...
        try:
            ...
            self.logger.log_runtime_status(
                "EXTERNAL_MONITORING_MANUAL_EXPORT_ERROR", f"Manual export failed: {str(e)}"
            )
            return False
