...
class StrategyManager:
    ...
    async def analyze_symbol(self, symbol: str, ohlcv_data: list[dict]) -> dict[str, Any] | None:
        ...
        try:
            ...
            min_volume = (
                getattr(self.config, "min_volume_24h_usdc", 5000000.0) / 24
            )  # Конвертируем дневной объем в часовой
            if avg_volume < min_volume:
                self.logger.log_event(
                    "STRATEGY", "DEBUG", f"Market invalid for {symbol}: Low volume"
                )
                return None
    ...
    def _calculate_signals(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        try:
            ...
            # Price momentum
            signals["price_change"] = (df["close"].iloc[-1] - df["close"].iloc[-5]) / df[
                "close"
            ].iloc[-5]
    ...
    def _should_enter_position(self, signals: dict[str, Any], symbol: str) -> bool:
        ...
        try:
            ...
            min_strength = getattr(
                self.config, "min_signal_strength", 1
            )  # Используем getattr с дефолтным значением
