...
class OrderManager:
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.logger.log_event(
                "ORDER_MANAGER", "ERROR", f"Failed to initialize OrderManager: {e}"
            )
            return False
                ...
                for pos in positions:
                    ...
                    if abs(size) > 0:  # Есть позиция
                        if symbol not in self.active_positions:
                            ...
                            self.logger.log_event(
                                "ORDER_MANAGER", "INFO", f"Found existing position: {symbol}"
                            )
                    else:
                        ...
                        if symbol in self.active_positions:
                            ...
                            self.logger.log_event(
                                "ORDER_MANAGER", "INFO", f"Position closed: {symbol}"
                            )
    ...
    async def place_position_with_tp_sl(
        ...
        try:
            ...
            main_order = await self.exchange.place_order(
                symbol=symbol, side=side, order_type="MARKET", amount=quantity
            )

            if main_order.get("status") not in ["FILLED", "closed"]:
                return {"success": False, "error": f"Main order failed: {main_order}"}
    ...
    async def place_tp_sl_orders(
        ...
        try:
            ...
            try:
                ...
                self.logger.log_event(
                    "ORDER_MANAGER", "WARNING", f"Failed to cancel old SL orders for {symbol}: {e}"
                )
            ...
            try:
                ...
                self.logger.log_event(
                    "ORDER_MANAGER", "WARNING", f"Failed to adjust SL for {symbol}: {e}"
                )
            ...
            for retry in range(sl_retry_limit):
                try:
                    ...
                    if retry < sl_retry_limit - 1:
                        ...
                        self.logger.log_event(
                            "ORDER_MANAGER", "ERROR", f"SL placement failed for {symbol}: {e}"
                        )
            ...
            if success_sl:
                ...
                self.logger.log_event(
                    "ORDER_MANAGER", "INFO", f"SL placed for {symbol} at ${sl_price:.4f}"
                )
            else:
                self.logger.log_event(
                    "ORDER_MANAGER",
            ...
            for i, (tp_price, tp_qty) in enumerate(tp_levels):
                try:
                    ...
                    self.logger.log_event(
                        "ORDER_MANAGER", "WARNING", f"Failed to place TP{i + 1} for {symbol}: {e}"
                    )
    ...
    def calculate_take_profit_levels(
        ...
        for i, (percent, size_ratio) in enumerate(
            zip(self.config.step_tp_levels, self.config.step_tp_sizes, strict=False)
        ):
            if side == "BUY":
    ...
    async def close_position_emergency(self, symbol: str) -> dict[str, Any]:
        ...
        try:
            ...
            close_order = await self.exchange.place_order(
                symbol=symbol, side=exit_side, order_type="MARKET", amount=position["qty"]
            )
    ...
    async def check_timeouts(self):
        ...
        for symbol, position in list(self.active_positions.items()):
            ...
            try:
                ...
                self.logger.log_event(
                    "ORDER_MANAGER", "ERROR", f"Failed to get price for {symbol}: {e}"
                )
                continue
        ...
        try:
            ...
            for position in positions:
                if (
                    ...
                    and abs(float(position.get("positionAmt", 0))) > 0
                ):
                    return True
            return False
        except Exception as e:
            self.logger.log_event(
                "ORDER_MANAGER", "ERROR", f"Ошибка проверки позиции {symbol}: {e}"
            )
            return False
    ...
    def reset_emergency_flag(self):
        ...
        self.logger.log_event(
            "ORDER_MANAGER", "INFO", "Emergency shutdown flag reset - position restoration enabled"
        )
