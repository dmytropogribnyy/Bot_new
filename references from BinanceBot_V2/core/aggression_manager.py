...
class AggressionManager:
    ...
    def set_aggression_level(self, level: str) -> bool:
        ...
        try:
            ...
            self.logger.log_event("AGGRESSION_MANAGER", "INFO",
                                f"Уровень агрессивности изменен на: {level.upper()}")
            return True

        except ValueError:
            self.logger.log_event("AGGRESSION_MANAGER", "ERROR",
                                f"Неверный уровень агрессивности: {level}")
            return False
        ...
        for strategy_name, strategy_settings in strategy_config.items():
            if 'aggressive_settings' in strategy_settings:
                ...
                should_enable_aggressive = (
                    self.current_level == AggressionLevel.AGGRESSIVE and
                    strategy_settings.get('aggression_mode') == 'AGGRESSIVE'
                )
    ...
    def get_strategy_settings(self, strategy_name: str) -> Dict[str, Any]:
        ...
        if strategy_name in strategy_config:
            ...
            if (self.current_level == AggressionLevel.AGGRESSIVE and
                'aggressive_settings' in settings):
    ...
    def analyze_market_conditions(self, market_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if (self.auto_switch_enabled and
                analysis['recommended_level'] != self.current_level.value and
            ...
            self.logger.log_event("AGGRESSION_MANAGER", "ERROR", f"Ошибка анализа рыночных условий: {e}")
                ...
                if aggressive.get('enabled', False):
                    ...
                    for param in required_params:
                        if param not in aggressive:
                            errors.append(f"Отсутствует параметр {param} в агрессивных настройках {strategy_name}")
