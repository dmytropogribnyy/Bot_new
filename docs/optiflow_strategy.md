...
...
🔧 1. adaptive_optimizer.py — Оптимизатор конфигураций
Цель:
Автоматически анализировать историю сделок (tp_performance.csv, balance и др.) и корректировать параметры стратегии в runtime_config.json:

TP уровни (step_tp_levels, sizes)

min_profit_threshold

risk_multiplier, base_risk_pct

возможно, адаптация таймфреймов, ATR, volume

Режим:
Фоновый процесс (cron / по расписанию / каждый вечер), может быть даже без запуска бота.

🎯 2. tp_sniper.py — Снайпер TP-моментов
Цель:
Находить сделки, где:

цена близка к TP1/TP2, но ещё не дошла

стратегия основного бота не позволяет войти (например, слабый сигнал)

но с коротким стопом и точкой входа это высоковероятностная сделка

Поведение:

Быстрый вход (market)

TP = ближайший уровень, SL = tight

Время удержания ≤ 5 минут

Особенности:

Может запускаться на 2–3 самых популярных монетах

Использует monitor_active_position, но агрессивнее

⚡ 3. hft_worker.py — Высокочастотный HFT-бот
Цель:
Работа на низких таймфреймах (15s–1m):

Скальп-сделки с быстрым входом и выходом

AutoProfit при +0.3–0.5%

SL tight, без TP1/TP2

Частые входы (до 20 в час)

Особенности:

Использует WebSocket-цену (через ws_price_monitor)

Мгновенное реагирование

Ведёт отдельный учёт позиций

🧠 Архитектура:
Каждый из этих агентов может:

иметь отдельный runtime_config_X.json

запускаться через отдельный tmux / pm2 процесс

использовать общие модули: enter_trade, monitor_active_position, check_entry_allowed, record_trade_result

различаться по bot_name, chat_id, логам, фильтрам
