...
import pandas as pd
import ta
from aggressiveness_controller_leg import get_aggressiveness_score
    ...
    try:
        ...
        if not DRY_RUN:
            try:
                ...
                from trade_engine import run_auto_profit_exit  # Ensure this is imported
...
def record_trade_result(symbol, side, entry_price, exit_price, result_type):
    ...
    if trade.get("tp1_hit", False) or trade.get("tp2_hit", False):
        ...
        exit_reason = "flat"  # neither TP nor SL
    ...
    # === Новое: логируем (win/lose) в component_tracker
    from core.component_tracker import log_component_data
    ...
    # Критерий успеха: exit_reason == "tp" + реальная прибыль
