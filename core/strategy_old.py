...
import pandas as pd
import ta
from trade_engine import trade_manager
...
def fetch_data_multiframe(symbol):
    ...
    import pandas as pd
    import ta
    from strategy import detect_ema_crossover, get_htf_trend
...
def passes_unified_signal_check(score, breakdown):
    """
    TEMP: отключена проверка сигнального комбо для отладки входов.
    """
    return True
...
def should_enter_trade(symbol, exchange, last_trade_times, last_trade_times_lock):
    ...
    from score_evaluator_leg import calculate_score
    from trade_engine import calculate_order_quantity, trade_manager
    ...
    from utils_core import (
        get_cached_balance,
        get_cached_symbol_open_interest,
        is_optimal_trading_hour,
        log_rejected_entry,
        log_score_history,
        send_telegram_message,
        update_cached_symbol_open_interest,
    )

    from . import get_enhanced_market_regime, passes_filters, passes_unified_signal_check
    ...
    if df is None:
        ...
        # Логируем отказ с score=0 и пустым breakdown
        log_rejected_entry(symbol, 0, failure_reasons, {})
    ...
    log(f"[DEBUG] Running should_enter_trade for {symbol} with score={score:.2f}", level="DEBUG")
    ...
    if score < min_required:
        ...
        if score > 0:
            log_component_data(symbol, breakdown, is_successful=False)

        # Логируем отказ
        log_rejected_entry(symbol, score, failure_reasons, breakdown)
        return None, failure_reasons
    ...
    # === log breakdown как “успешный сигнал” (по умолчанию is_successful=True)
    log_component_data(symbol, breakdown)

    # Проверка наличия MACD-колонок
    if "macd" not in df.columns or "macd_signal" not in df.columns:
        failure_reasons.append("missing_indicators")
        log(f"[{symbol}] macd or macd_signal missing", level="WARNING")
        if score > 0:
            log_component_data(symbol, breakdown, is_successful=False)
    ...
    tp1, tp2, sl_price, share1, share2 = calculate_tp_levels(entry_price, direction, regime, score, df)
    if tp1 is None or sl_price is None:
    ...
    # Если всё ок — обновляем last_trade_times
    with last_trade_times_lock:
        last_trade_times[symbol] = utc_now

    # финальный возврат (команда на вход)
    if not DRY_RUN:
        log_score_history(symbol, score)
