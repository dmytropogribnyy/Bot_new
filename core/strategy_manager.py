...
class StrategyManager:
    ...
    def __init__(self, config: TradingConfig, exchange: OptimizedExchangeClient,
                 symbol_manager: SymbolManager, logger: UnifiedLogger):
        self.config = config
        ...
        self.strategies = {
            "scalping_v1": ScalpingV1(config, logger)
        }
        ...
        self.logger.log_event("STRATEGY_MANAGER", "INFO", f"Strategy manager initialized with active strategy: {self.active_strategy}")

    def get_active_strategy(self):
        """Get the currently active strategy"""
        return self.strategies.get(self.active_strategy)

    async def evaluate_symbol(self, symbol: str) -> Tuple[Optional[str], Dict[str, Any]]:
        """
        ...
        try:
            ...
            if direction:
                ...
                self.logger.log_event("STRATEGY_MANAGER", "INFO",
                    f"{symbol}: Signal generated by {self.active_strategy} - {direction}")
    ...
    async def _fetch_ohlcv_data(self, symbol: str, timeframe: str = "5m", limit: int = 100) -> Optional[pd.DataFrame]:
