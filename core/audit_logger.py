...
class AuditLogger:
    ...
    def _redact(self, data: Any) -> Any:
        """Redact sensitive keys in a case-insensitive manner with common variants.

        Examples of redacted keys: apiKey, API_KEY, secretKey, password, token, listenKey, etc.
        """
        sensitive_patterns = (
            "api_key",
            "apikey",
        ...
        def mask(obj: Any) -> Any:
            if isinstance(obj, dict):
                return {k: ("***" if is_sensitive_key(k) else mask(v)) for k, v in obj.items()}
