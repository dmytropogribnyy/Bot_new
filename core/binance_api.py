# binance_api.py (финальная версия)
...
def fetch_ohlcv(symbol, timeframe="15m", limit=100):
    ...
    return safe_call_retry(lambda: exchange.fetch_ohlcv(symbol, timeframe, limit=limit), label=f"fetch_ohlcv {symbol}")


def create_market_order(symbol, side, amount):
    """Создает рыночный ордер."""
    return safe_call_retry(
        lambda: exchange.create_market_buy_order(symbol, amount) if side == "buy" else exchange.create_market_sell_order(symbol, amount),
...
def cancel_order(order_id, symbol):
    ...
    return safe_call_retry(lambda: exchange.cancel_order(order_id, symbol), label=f"cancel_order {symbol}")


def fetch_open_orders(symbol):
    """Получает открытые ордера для символа."""
    return safe_call_retry(lambda: exchange.fetch_open_orders(symbol), label=f"fetch_open_orders {symbol}")
...
# Add these new functions to binance_api.py
...
def calculate_commission(order_size, price, is_maker=False):
    ...
    Args:
        ...
        is_maker: Whether the order is maker (True) or taker (False)

    Returns:
        Commission amount in USDC
    """
    from common.config_loader import MAKER_FEE_RATE, TAKER_FEE_RATE
