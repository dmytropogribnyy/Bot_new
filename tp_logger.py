    ...
    if not os.path.exists(EXPORT_PATH):
        with open(EXPORT_PATH, mode='w', newline='') as f:
            ...
            writer.writerow([
                ...
                "atr", "adx", "bb_width", "price"
            ])

def log_trade_result(
    symbol, side, entry_price, exit_price,
    tp1_hit, tp2_hit, sl_hit, pnl_percent, result, duration_minutes,
    atr=None, adx=None, bb_width=None, price=None
):
    ensure_log_exists()
    with open(EXPORT_PATH, mode='a', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([
            datetime.now(TIMEZONE).strftime("%Y-%m-%d %H:%M"), symbol, side,
            round(entry_price, 5), round(exit_price, 5),
            "YES" if tp1_hit else "NO",
            "YES" if tp2_hit else "NO",
            "YES" if sl_hit else "NO",
            round(pnl_percent, 2), result, round(duration_minutes, 1),
