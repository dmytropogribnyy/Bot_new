...
def evaluate_best_config(days=7):
    if not os.path.exists(EXPORT_PATH):
        send_telegram_message("❌ No trade history available for optimization.", force=True)
        return
    ...
    try:
        ...
        if abs(new_tp1 - TP1_PERCENT) / TP1_PERCENT > UPDATE_THRESHOLD or abs(new_tp2 - TP2_PERCENT) / TP2_PERCENT > UPDATE_THRESHOLD:
            ...
            send_telegram_message(f"✅ TP1/TP2 auto-updated:\nTP1: {round(new_tp1*100, 2)}%\nTP2: {round(new_tp2*100, 2)}%", force=True)
...
def _update_filter_thresholds(new_thresholds: dict):
    try:
        ...
        if changes:
            ...
            send_telegram_message(report, force=True)
        else:
...
def _analyze_filter_thresholds():
    try:
        ...
        send_telegram_message(f"❌ Filter optimizer error: {e}")
...
def _analyze_symbol_stats():
    try:
        ...
        if messages:
            send_telegram_message("📌 Symbol Stats:\n" + "\n".join(messages), force=True)
